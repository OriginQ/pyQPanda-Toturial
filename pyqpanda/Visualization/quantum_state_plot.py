...
def config_colors(x, y, z, dx, dy, dz, color):
    ...
    Args:
        ...
        color (list or np.ndarray): 
            The color to be applied to the cuboid faces. 
...
def config_normals(polygons):
    ...
    Args:
        polygons (np.ndarray or list): 
            An array or list of lists containing the vertices of the polygons.
    ...
    Returns:
        np.ndarray: 
            An array of shape (N, 3) containing the normals of the input polygons.
...
def config_shade_colors(color, normals, lightsource=None):
    ...
    Args:
        color (numpy.ndarray): 
            An array of colors, where each color is represented as an RGB tuple
            or an RGBA tuple.
        normals (numpy.ndarray): 
            An array of normal vectors, where each vector is represented as a
            three-element tuple or array.
        lightsource (LightSource, optional): 
            A LightSource object representing the light source
            direction. Defaults to a light source with a direction
            of (225, 19.4712) degrees.

    Returns:
        numpy.ndarray: 
            The color array with shading applied. If no shading is applicable, the original
